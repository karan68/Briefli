...
pub fn calculate_buffer_timeout(
    ...
    // Calculate the buffer duration with 2x jitter headroom using integer
    // nanoseconds so exact frame/rate ratios do not drift below their target.
    let with_headroom_nanos = (u64::from(buffer_size) * 2 * 1_000_000_000) / u64::from(sample_rate);
    let with_headroom = Duration::from_nanos(with_headroom_nanos);
